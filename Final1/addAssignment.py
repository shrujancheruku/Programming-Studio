...
def main():
    collection = MongoClient()["final1db"]["assignments"]
    ...
    try:
        collection.insert({"_id": assign_name, "url": "#", 'parent_class_id': my_class.get('_id'), 'grade_id_list': []})
        print("Assignment created.")
    except DuplicateKeyError:
        print("Assignment already present in DB.")

    assign = collection.find_one({"_id": assign_name})
